...
//Addressing Modes:
        ...
        void implicit();  uint8_t immediate(); uint8_t zero_page_y(); uint8_t absolute_x(); uint8_t accumulator();
        uint8_t zero_page(); uint8_t relative(); uint8_t absolute_y(); uint8_t inderect_indexed();
...
//Instructions:
    ...
    void AND(uint8_t data); void BEQ(uint8_t data); void BPL(uint8_t data); void CMP(uint8_t data);void CLS(); void ORA(uint8_t data); void ROR(uint8_t data); void DEX(); void INX(); void JSR(); void PHP(); void BRK(); void STY(); void TXA();
