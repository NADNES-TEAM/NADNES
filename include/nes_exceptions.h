...
namespace NES {
    ...
    struct InvalidMapperConfigurationError : CartridgeError {
        [[nodiscard]] InvalidMapperConfigurationError() : CartridgeError("Invalid mapper configuration") {};
    };
    ...
    struct AddressOutOfBoundsError : CartridgeError {
        [[nodiscard]] explicit AddressOutOfBoundsError(uint16_t address, const std::string &type) : CartridgeError(
                "Address '" + std::to_string(address) + "' can't be mapped to " + type + " address space") {
        }
    };
